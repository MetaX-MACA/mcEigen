...
void test_nnls_with_half_precision() {
  ...
  using Mat = Matrix<Eigen::half, 8, 2>;
  using VecB = Matrix<Eigen::half, 8, 1>;
  using VecX = Matrix<Eigen::half, 2, 1>;
  Mat A = Mat::Random();  // full-column rank with high probability.
  VecB b = VecB::Random();

  NNLS<Mat> nnls(A, 20, Eigen::half(1e-2f));
  const VecX x = nnls.solve(b);

  VERIFY_IS_EQUAL(nnls.info(), ComputationInfo::Success);
  verify_nnls_optimality(A, b, x, Eigen::half(1e-1));
}
