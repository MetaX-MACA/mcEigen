...
__device__ EIGEN_ALWAYS_INLINE void atomicReduce(T* output, T accum, R& reducer) {
  ...
  (defined(EIGEN_MACA_ARCH) && defined(__MC_ARCH_HAS_WARP_SHUFFLE__))
  if (sizeof(T) == 4)
  {
...
#pragma unroll
  ...
  #elif defined(EIGEN_MACACC)
    if (std::is_floating_point<typename Self::CoeffReturnType>::value) {
      reducer.reduce(__shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, static_cast<float>(accum), offset, warpSize), &accum);
    } else {
      reducer.reduce(__shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, static_cast<int>(accum), offset, warpSize), &accum);
    }
...
#if defined(EIGEN_HIPCC) || defined(EIGEN_MACACC)
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionInitFullReduxKernelHalfFloat(
    Reducer reducer, const Self input, Index num_coeffs, Eigen::half* scratch) {
  ...
  if (packet_remainder != 0) {
    ...
    if ((num_coeffs & 1) != 0) {
      Eigen::half lastCoeff = input.coeff(num_coeffs - 1);
...
template <typename Self,
          typename Reducer, typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionInitKernelHalfFloat(Reducer reducer, const Self /*input*/, Index num_coeffs, Eigen::half* output) {
  const Index thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  const Index num_threads = blockDim.x * gridDim.x;
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void FullReductionKernelHalfFloat(
    ...
    Eigen::half* output, Eigen::half* scratch) {
  typedef typename packet_traits<Eigen::half>::type PacketType;
  ...
  if (gridDim.x == 1) {
    if (first_index == 0) {
      ...
      if (rem != 0) {
        ...
        if ((num_coeffs & 1) != 0) {
          Eigen::half last = input.coeff(num_coeffs - 1);
...
#pragma unroll
  for (int offset = warpSize/2; offset > 0; offset /= 2) {
    ...
    for (int i = 0; i < packet_width / 2; i++) {
      ...
      union { int i; half2 h; } wka_in, wka_out;
      wka_in.h = hacc[i];
      wka_out.i = __shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, wka_in.i, offset, warpSize);
      hr[i] = wka_out.h;
    }
    reducer.reducePacket(r1, &accum);
  #elif defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionCleanupKernelHalfFloat(Op reducer, Eigen::half* output, Eigen::half* scratch) {
  ...
  if (unpacket_traits<packet_type>::size == 1) {
    *output = *scratch;
    ...
    Eigen::half tmp = __float2half(0.f);
...
struct FullReductionLauncher<Self, Op, Eigen::half, false> {
  static void run(const Self&, Op&, const GpuDevice&, Eigen::half*, typename Self::Index) {
...
struct FullReductionLauncher<Self, Op, Eigen::half, true> {
  static void run(const Self& self, Op& reducer, const GpuDevice& device, Eigen::half* output, typename Self::Index num_coeffs) {
    typedef typename Self::Index Index;

    const int block_size = 256;
    const int num_per_thread = 128;
    const int num_blocks = divup<int>(num_coeffs, block_size * num_per_thread);
    Eigen::half* scratch = static_cast<Eigen::half*>(device.scratchpad());
...
#pragma unroll
      ...
      #elif defined(EIGEN_MACACC)
        if (std::is_floating_point<Type>::value) {
          reducer.reduce(__shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, static_cast<float>(reduced_val), offset), &reduced_val);
        } else {
          reducer.reduce(__shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, static_cast<int>(reduced_val), offset), &reduced_val);
        }
      #elif defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000
      ...
      for (Index j = 0; j < NumPerThread; j += unroll_times) {
        ...
        if (last_col >= num_coeffs_to_reduce) {
          ...
          if (col < num_coeffs_to_reduce) {
            ...
            if (col < num_coeffs_to_reduce) {
              // Peel;
              const Eigen::half last1 =
                  input.m_impl.coeff(row * num_coeffs_to_reduce + col);
              *hr1 = __halves2half2(last1, reducer.initialize());
              const Eigen::half last2 =
                  input.m_impl.coeff((row + 1) * num_coeffs_to_reduce + col);
...
#pragma unroll
	  ...
	  hr2[i] = wka_out2.h;
        ...
        for (int i = 0; i < packet_width / 2; i++) {
          ...
          union { int i; half2 h; } wka_in1, wka_out1;
          wka_in1.h = rv1[i];
          wka_out1.i = __shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, wka_in1.i, offset, warpSize);
          hr1[i] = wka_out1.h;

          union { int i; half2 h; } wka_in2, wka_out2;
          wka_in2.h = rv2[i];
          wka_out2.i = __shfl_down_sync(0xFFFFFFFFFFFFFFFFULL, wka_in2.i, offset, warpSize);
          hr2[i] = wka_out2.h;
        }
        reducer.reducePacket(r1, &reduced_val1);
        reducer.reducePacket(r2, &reduced_val2);
...
struct InnerReductionLauncher<Self, Op, Eigen::half, false> {
  static bool run(const Self&, Op&, const GpuDevice&, Eigen::half*, typename Self::Index, typename Self::Index) {
    gpu_assert(false && "Should not be called since there is no packet accessor");
    return true;
  }
};
...
struct InnerReductionLauncher<Self, Op, Eigen::half, true> {
  static bool run(const Self& self, Op& reducer, const GpuDevice& device, Eigen::half* output, typename Self::Index num_coeffs_to_reduce, typename Self::Index num_preserved_vals) {
    typedef typename Self::Index Index;
