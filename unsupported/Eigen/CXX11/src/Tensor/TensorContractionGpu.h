...
#if defined(EIGEN_HIPCC) || (defined(EIGEN_CUDACC) && defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000)
#define shuffleInc(i, j, mask) res(i, j) += __shfl_xor(res(i, j), mask)
#elif defined(EIGEN_MACACC)
#define shuffleInc(i, j, mask) res(i, j) += __shfl_xor_sync(0xFFFFFFFFFFFFFFFFULL, res(i, j), mask)
#else
...
#else // for EIGEN_CUDACC || EIGEN_MACACC
...
#define prefetch_lhs(reg, row, col)                            \
  ...
  for (Index k = 0; k < k_size; k += 16) {
    ...
    x2 = __shfl_xor(x2, 4);
    #elif defined(EIGEN_MACACC)
    x1 = __shfl_xor_sync(0xFFFFFFFFFFFFFFFFULL, x1, 4);
    x2 = __shfl_xor_sync(0xFFFFFFFFFFFFFFFFULL, x2, 4);
    #else
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType, OutputKernelType>, GpuDevice> :
  ...
  void evalTyped(Scalar* buffer) const {
    ...
    setGpuSharedMemConfig(mcSharedMemBankSizeEightByte);
#else
