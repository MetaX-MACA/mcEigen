...
macro(ei_add_test_internal testname testname_with_suffix)
  ...
  if(EIGEN_ADD_TEST_FILENAME_EXTENSION STREQUAL cu)
    set(is_gpu_test ON)
    if(EIGEN_TEST_MACA)
      eigen_wrap_cu_in_cpp(realfilename ${filename} ${targetname})
      # add_executable(${targetname} "${realfilename}"  "-DEIGEN_USE_MACA ${ARGV2}")
      ...
      add_executable(${targetname} ${realfilename})
...
macro(ei_add_test testname)
  ...
  set(prefixed_testname test_eigen_${testname})
  set(EIGEN_TESTS_LIST "${EIGEN_TESTS_LIST}${prefixed_testname}\n")
  ...
  if( (EIGEN_SPLIT_LARGE_TESTS AND suffixes) OR explicit_suffixes)
    ...
    foreach(suffix ${suffixes})
      ei_add_test_internal(${testname} ${prefixed_testname}_${suffix} "${ARGV1}" "${ARGV2}")
      add_dependencies(${testname} ${prefixed_testname}_${suffix})
      target_compile_definitions(${prefixed_testname}_${suffix} PRIVATE -DEIGEN_TEST_PART_${suffix}=1)
    endforeach()
  else()
    ei_add_test_internal(${testname} ${prefixed_testname} "${ARGV1}" "${ARGV2}")
    target_compile_definitions(${prefixed_testname} PRIVATE -DEIGEN_TEST_PART_ALL=1)
  endif()
endmacro()
...
macro(ei_testing_print_summary)
  ...
  message(STATUS "Maximal matrix/vector size: ${EIGEN_TEST_MAX_SIZE}")
    ...
    if(EIGEN_TEST_MACA)
      message(STATUS "MACA:              ON (using mxcc)")
    else()
      message(STATUS "MACA:              OFF")
    endif()
