...
#if defined(EIGEN_HIP_DEVICE_COMPILE) || (defined(EIGEN_CUDA_ARCH) && EIGEN_CUDA_ARCH >= 350) || defined(EIGEN_MACA_ARCH)
...
#if defined(EIGEN_HIP_DEVICE_COMPILE) || defined(EIGEN_CUDA_HAS_FP16_ARITHMETIC) || defined(EIGEN_MACA_ARCH)
...
#if defined(EIGEN_CUDA_ARCH) || defined(EIGEN_HIPCC) || (defined(EIGEN_CUDACC) && EIGEN_COMP_CLANG && !EIGEN_COMP_NVCC) || defined(EIGEN_MACACC)
...
#endif // defined(EIGEN_CUDA_ARCH) || defined(EIGEN_HIPCC) || (defined(EIGEN_CUDACC) && EIGEN_COMP_CLANG && !EIGEN_COMP_NVCC) || defined(EIGEN_MACACC)
...
// emulated.
#if (defined(EIGEN_HAS_CUDA_FP16) || defined(EIGEN_HAS_HIP_FP16) || defined(EIGEN_HAS_MACA_FP16)) && defined(EIGEN_GPU_COMPILE_PHASE)
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE half2 pabs(const half2& a) {
  Eigen::half a1 = __low2half(a);
  Eigen::half a2 = __high2half(a);
  Eigen::half result1 = half_impl::raw_uint16_to_half(a1.x & 0x7FFF);
  Eigen::half result2 = half_impl::raw_uint16_to_half(a2.x & 0x7FFF);
  return __halves2half2(result1, result2);
}

EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE half2 ptrue(const half2& /*a*/) {
  Eigen::half true_half = half_impl::raw_uint16_to_half(0xffffu);
  return pset1<half2>(true_half);
}

EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE half2 pzero(const half2& /*a*/) {
  Eigen::half false_half = half_impl::raw_uint16_to_half(0x0000u);
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE half2 pselect(const half2& mask,
                                                    const half2& a,
                                                    const half2& b) {
  Eigen::half mask_low = __low2half(mask);
  Eigen::half mask_high = __high2half(mask);
  Eigen::half result_low = mask_low == Eigen::half(0) ? __low2half(b) : __low2half(a);
  Eigen::half result_high = mask_high == Eigen::half(0) ? __high2half(b) : __high2half(a);
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE half2 pcmp_eq(const half2& a,
  ...
  Eigen::half a1 = __low2half(a);
  Eigen::half a2 = __high2half(a);
  Eigen::half b1 = __low2half(b);
  Eigen::half b2 = __high2half(b);
  Eigen::half eq1 = __half2float(a1) == __half2float(b1) ? true_half : false_half;
  Eigen::half eq2 = __half2float(a2) == __half2float(b2) ? true_half : false_half;
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE half2 pand(const half2& a,
  ...
  Eigen::half b1 = __low2half(b);
  Eigen::half b2 = __high2half(b);
  Eigen::half result1 = half_impl::raw_uint16_to_half(a1.x & b1.x);
  Eigen::half result2 = half_impl::raw_uint16_to_half(a2.x & b2.x);
  return __halves2half2(result1, result2);
}
...
#if defined(EIGEN_HIP_DEVICE_COMPILE) || defined(EIGEN_MACA_ARCH)
...
#endif // (defined(EIGEN_HAS_CUDA_FP16) || defined(EIGEN_HAS_HIP_FP16) || defined(EIGEN_HAS_MACA_FP16)) && defined(EIGEN_GPU_COMPILE_PHASE)
