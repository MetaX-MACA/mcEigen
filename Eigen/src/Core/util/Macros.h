...
#if defined(__NVCC__) && !defined(__MACACC__)
...
#if defined(__NVCC__) && !defined(__MACACC__) && defined(__HIPCC__)
  #error "NVCC as the target platform for HIPCC is currently not supported."
#endif

#if defined(__CUDACC__) && !defined(EIGEN_NO_CUDA) && !defined(__MACACC__)
  // Means the compiler is either nvcc or clang with CUDA enabled
  #define EIGEN_CUDACC __CUDACC__
#endif

#if defined(__CUDA_ARCH__) && !defined(EIGEN_NO_CUDA) && !defined(__MACA_ARCH__)
...
#if defined(__MACACC__) && !defined(EIGEN_NO_MACA)
  // Means the compiler is MXCC (analogous to EIGEN_CUDACC, but for MACA)
  #define EIGEN_MACACC __MACACC__
  #include <mc_runtime.h>
  #if defined(__MACA_ARCH__)
    // analogous to EIGEN_CUDA_ARCH, but for MACA
    #define EIGEN_MACA_ARCH __MACA_ARCH__
  #endif
...
// Unify CUDA/HIPCC/MACACC

#if defined(EIGEN_CUDACC) || defined(EIGEN_HIPCC) || defined(EIGEN_MACACC)
//
// If either EIGEN_CUDACC or EIGEN_HIPCC/EIGEN_MACACC is defined, then define EIGEN_GPUCC
//
#define EIGEN_GPUCC
//
...
// EIGEN_MACACC implies the MACA compiler and is used to tweak Eigen code for use in MACA kernels
//
// In most cases the same tweaks are required to the Eigen code to enable in both the HIP/MACA and CUDA kernels.
...
//      #if defined(EIGEN_CUDACC)
//
// For cases where the tweak is specific to MACA, the code should be guarded with
//      #if defined(EIGEN_MACACC)
//
#endif

#if defined(EIGEN_CUDA_ARCH) || defined(EIGEN_HIP_DEVICE_COMPILE) || defined(EIGEN_MACA_ARCH)
//
// If either EIGEN_CUDA_ARCH or EIGEN_HIP_DEVICE_COMPILE/EIGEN_MACA_ARCH is defined, then define EIGEN_GPU_COMPILE_PHASE
//
#define EIGEN_GPU_COMPILE_PHASE
//
// GPU compilers (HIPCC, NVCC, MXCC) typically do two passes over the source code,
...
// EIGEN_MACA_ARCH implies the device compilation phase in MACA
//
// In most cases, the "host" / "device" specific code is the same for both HIP/MACA and CUDA
...
// All functions callable from CUDA/HIP/MACA code must be qualified with __device__
...
// When compiling CUDA/HIP/MACA device code with NVCC or HIPCC/MXCC
// pull in math functions from the global namespace.
// In host mode, and when device code is compiled with clang,
// use the std versions.
#if (defined(EIGEN_CUDA_ARCH) && defined(__NVCC__)) || defined(EIGEN_HIP_DEVICE_COMPILE) || defined(EIGEN_MACA_ARCH)
...
#if (defined(_CPPUNWIND) || defined(__EXCEPTIONS)) && !defined(EIGEN_CUDA_ARCH) && !defined(EIGEN_EXCEPTIONS) && !defined(EIGEN_USE_SYCL) && !defined(EIGEN_HIP_DEVICE_COMPILE) && !defined(EIGEN_MACA_ARCH)
...
#    define EIGEN_THROW_X(X) __builtin_trap()
#    define EIGEN_THROW __builtin_trap()
#  else
